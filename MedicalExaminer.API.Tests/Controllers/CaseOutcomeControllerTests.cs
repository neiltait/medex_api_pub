...
namespace MedicalExaminer.API.Tests.Controllers
{
    public class CaseOutcomeControllerTests : AuthorizedControllerTestsBase<CaseOutcomeController>
    {
        ...
        public async void PutWaiveFee_When_Called_When_ModeOfDisposal_Is_Not_Cremation_Returns_Bad_Request()
        {
            ...
            var confirmationOfScrutinyService =
                new Mock<IAsyncQueryHandler<ConfirmationOfScrutinyQuery, Examination>>();
            var saveWaiveFeeService = new Mock<IAsyncQueryHandler<SaveWaiveFeeQuery, Examination>>();
            var voidCaseService = new Mock<IAsyncQueryHandler<VoidCaseQuery, Examination>>();
        ...
        [Fact]
        public async void PutVoidCase_When_Called_With_Valid_Examination_Id_Returns_Ok()
        {
            // Arrange
            var logger = new Mock<IMELogger>();
            var mapper = new Mock<IMapper>();
            ...
            var putVoidCaseRequest = new PutVoidCaseRequest()
            {
                VoidReason = "VoidReason"
            };

            // Act
            var response = await sut.PutVoidCase(examination.ExaminationId, putVoidCaseRequest);

            // Assert
            var taskResult = response.Should().BeOfType<ActionResult<PutVoidCaseResponse>>().Subject;
        }

        [Fact]
        public async void PutVoidCase_When_Called_With_Invalid_Case_Id_Returns_Bad_Request()
        {
            // Arrange
            var logger = new Mock<IMELogger>();
            var mapper = new Mock<IMapper>();
            ...
            var taskResult = response.Should().BeOfType<ActionResult<PutVoidCaseResponse>>().Subject;
            var badRequestResult = taskResult.Result.Should().BeAssignableTo<BadRequestObjectResult>().Subject;
            badRequestResult.Value.Should().BeAssignableTo<PutVoidCaseResponse>();
        }

        [Fact]
        public async void PutVoidCase_When_Called_With_null_Case_Id_Returns_Bad_Request()
        {
            // Arrange
            var logger = new Mock<IMELogger>();
            var mapper = new Mock<IMapper>();
        ...
        [Fact]
        public async void PutVoidCase_When_Called_With_Valid_But_Not_Found_Case_Id_Returns_Bad_Request()
        {
            // Arrange
            var logger = new Mock<IMELogger>();
            var mapper = new Mock<IMapper>();
        ...
        [Fact]
        public async void PutVoidCase_When_Called_With_Valid_Case_Id_And_Invalid_User_Returns_Forbid()
        {
            // Arrange
            SetupAuthorize(AuthorizationResult.Failed());
            var logger = new Mock<IMELogger>();
